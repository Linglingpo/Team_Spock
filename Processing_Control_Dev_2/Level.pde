class Level {
  ...
  void display() {
    ...
    if (scene == 0) {
      displayIntro ();
      if (nowSpockOn) {
        holdingTime = millis()/1000 - counter;
        scene = 3;
        nowSpockOn = false;
      }
    ...
    //-- into --//
      ...
      holdingTime++;
      controlAnimation(holdingTime);
      //println(holdingTime);
      if (nowSpockOn && holdingTime >= 50) {
        scene = 1;
        nowSpockOn = false;
      }
    }
  ...
  //-- Control --//
  void controlAnimation(float _tempTime) {
    float remainder = _tempTime % 20;
    println(remainder); 
    if (remainder <= 10 ) {
      image(noSelectImg, 0, 0, width, height);
    } else {
      image(selectImg, 0, 0, width, height);
    }
  }
  ...
  void displayLevel01() {
    ...
    if (nowSpockOn) {
      ...
      if (foodLvl01.timertest1*2 >= foodLvl01.timertest) {
        ...
        image(checkImg, 290, 280, 280, 280);
      }     
      nowSpockOn = false;
    }
  }
